        ...
        for file_policy in try(domain.policies.file_policies, []) : [
          {
            ...
            # Optional
            block_encrypted_archives     = try(file_policy.block_encrypted_archives, null)
            block_uninspectable_archives = try(file_policy.block_uninspectable_archives, null)
            clean_list                   = try(file_policy.clean_list, null)
            custom_detection_list        = try(file_policy.custom_detection_list, null)
            description                  = try(file_policy.description, null)
            first_time_file_analysis     = try(file_policy.first_time_file_analysis, null)
            inspect_archives             = try(file_policy.inspect_archives, null)
            max_archive_depth            = try(file_policy.max_archive_depth, null)
            threat_score                 = try(file_policy.threat_score, null)
            ...
            file_rules = [for file_rule in try(file_policy.file_rules, []) : {
              ...
              # Optional
              file_type_categories = try(file_rule.file_type_categories, null)
              file_types           = try(file_rule.file_types, null)
      ...
      targets = flatten([ 
        for domain in local.domains : [
          for device in try(domain.devices.devices, []) : [
            ...
            after_destroy_policy_id = local.local.map_access_control_policies[after_destroy_policy_name].id
            type = "Device"
            } 
